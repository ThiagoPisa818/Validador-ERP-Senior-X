...
export default function ValidadorSeniorX() {
  ...
  const handleDownloadCorrected = () => {
    ...
    const blob = new Blob([validationResult.correctedCsv], { type: "text/csv;charset=utf-8" })
  ...
  const handleDownloadCorrectedWithBOM = () => {
    ...
    const blob = new Blob([validationResult.correctedCsv], { type: "text/csv;charset=utf-8-bom" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url
    a.download = `${filename}_BOM.csv`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
                    ...
                    {uploadedFile ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">CSV, Excel (.xlsx, .xls) - Máx: 10MB</p>
                        </div>
                        <Button
                          type="button"
                          variant="outline"
