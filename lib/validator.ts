  ...
  pessoas: {
    ...
    validations: {
      CODIGO: (value, row, allRows) => {
        if (!value) {
          // Find next available code starting from 50
          const existingCodes = allRows
            .map((r) => Number.parseInt(r.CODIGO))
        ...
        const duplicateCount = allRows.filter((r) => r.CODIGO === value).length
        if (duplicateCount > 1) {
          // Find next available code starting from 50
          const existingCodes = allRows
            .map((r) => Number.parseInt(r.CODIGO))
      ...
      REGIME_ESPECIAL_TRIBUTACAO: (value) => {
        ...
        // If value is not understood, exclude the information (return empty)
        return ""
      },
      DATA_NASCIMENTO: (value, row) => {
        if (row?.TIPO_PESSOA === "JURIDICA") return ""
        return value // Keep filled when TIPO_PESSOA is FISICA
      },
